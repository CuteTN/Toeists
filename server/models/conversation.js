...
conversationSchema.pre("validate", async function (next) {
  if (this.type === "private") {
    if (this.members?.length !== 2)
      return next(
        this.invalidate(
          "members",
          "A private conversation must have exact 2 members."
        )
      );

    const existedConversation = await getPrivateConversation(
      this.members[0]?.memberId,
      this.members[1]?.memberId
    );

    if (existedConversation && !existedConversation._id.equals(this._id))
      return next(
        this.invalidate(
          "members",
          "The private conversation of these 2 members has already existed."
        )
      );
  }
  ...
  if (this.type === "group") {
    if (!this.name)
      return next(
        this.invalidate("name", "A group conversation must have a name.")
      );
    ...
    if (!this.members.some((member) => member.role === "admin"))
      return next(
        this.invalidate(
          "members",
          "A group conversation must have at least 1 admin."
        )
      );
  }
