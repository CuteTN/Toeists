...
function NotificationList({ notifications }) {
  ...
  const MenuNotificationsActions = () => (
    <Menu>
      <Menu.Item
        key="filterUnseen"
        onClick={handleToggleFilterUnseenNotificationsClick}
      >
      ...
      <Menu.Item
        key="markAllSeen"
        onClick={handleMarkAllNotificationsAsSeenClick}
      >
        <Row align="middle">
      ...
      <Menu.Item
        key="clearSeenNoti"
        onClick={handleClearSeenNotificationsClick}
      >
        <Row align="middle">
  ...
  const MenuNotificationActions = (notification) => (
    <Menu>
      <Menu.Item
        key="setSeen"
        onClick={() => {
          handleToggleNotificationSeenState(notification);
        }}
      >
        {notification?.isSeen ? (
          <Row align="middle">
      ...
      <Menu.Item
        key="removeNoti"
        onClick={() => handleRemoveNotificationsClick(notification)}
      >
        <Row align="middle">
  ...
  return (
    <Menu>
      ...
      {notificationsToShow?.length === 0 ? (
        <Menu.Item key="no-data" className="whiteorange-button">
          <div
            className="justify-content-center align-items-center p-2 text-center"
            style={{ width: 300 }}
          >
        ...
        <>
          {notificationsToShow?.slice(0, 5).map((noti, i) => (
            <Menu.Item key={noti._id} className="whiteorange-button">
              <div
              ...
              >
                ...
                >
                  ...
                  <Tooltip title={noti.text} placement="bottom">
                    <Text>{limitNameLength(noti.text, 54)}</Text>
                  </Tooltip>
